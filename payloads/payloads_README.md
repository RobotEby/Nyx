# Nyx Payloads – MITM Tips & Tricks

This folder contains real payloads delivered by Nyx's MITM HID Injection module (Option 11).
...
All payloads are delivered via http://10.0.0.1 by Nyx's MITM module.
Logs from keystrokes and page visits are saved to loot/keystroke*log*_.txt and loot/session*log*_.txt.
You do not need to change the IP — Nyx handles it. Just make sure you're listening or serving the next stage where needed.
...
3.  Configure the Implant: Before deploying rogue_implant.py via Nyx, edit the file to point to your C2 server. Update these lines:
    ```python
...
### How to Use These Payloads in Nyx

1.  **Place the Files**: Ensure all desired payloads are in the Nyx/payloads/ directory.
...
3.  **Launch Nyx MITM Attack**:
    - Run Nyx: sudo python3 nyx.py
    - Choose Option 11 (MITM HID Injection).
    - Select your target payload from the list.
    - Nyx handles the rest, serving the file from http://10.0.0.1.
...
- **C2 Reliability**: If using rogue_implant.py, ensure your ROGUE C2 server is stable and the ngrok URL is correctly configured in the implant before deploying it via Nyx.
