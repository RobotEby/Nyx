...
def require_root():
    if os.geteuid() != 0:
        print("⚠️ Nyx must be run as root.")
        sys.exit(1)
...
log-dhcp
    ...
    subprocess.run(["sudo", "pkill", "-f", "nyx_dns_spoofer.py"])
    time.sleep(1)

    print("Launching Nyx DNS spoofer...")
    subprocess.Popen(["python3", "nyx_dns_spoofer.py"])
